...
function detectPlatformCollision() {
  ...
  function drawHighScoreBox() {
    drawRoundedRect(canvas.width - 205, 5, 200, 35, 5, 'white', 'black', 3);
    ctx.fillStyle = 'black';
    ctx.font = '20px Arial';
    ctx.fillText(`Meilleur score: ${highScore}`, canvas.width - 200, 30);
  }

function drawRoundedRectangle(x, y, width, height, radius, fillColor, strokeColor) {
  ctx.beginPath();
  ...
  // Stroke
  if (strokeColor) {
    ctx.lineWidth = 2; // Taille du bord
...
function drawGameOverAndReset() {
  if (gameOver) {
    ...
    // Dessiner un rectangle arrondi avec un bord blanc
    drawRoundedRectangle(canvas.width / 2 - 100, canvas.height / 2 + 20, 210, 40, 20, 'orange', 'white');

    ctx.fillStyle = 'white';
    ctx.font = '20px Arial';
    ctx.fillText('Nouvelle partie', canvas.width / 2 - 65, canvas.height / 2 + 45);
  }
}
